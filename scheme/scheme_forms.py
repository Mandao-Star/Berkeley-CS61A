...
def do_define_form(expressions, env, tail = False):
    ...
    if scheme_symbolp(signature):
        ...
        define_expr = Pair('define', Pair(function_name, Pair(lambda_expr, nil)))
        return do_define_form(define_expr.rest, env)
        # END PROBLEM 10
    else:
...
def do_begin_form(expressions, env, tail = False):
    ...
    return eval_all(expressions, env, tail=True)
...
def do_and_form(expressions, env, tail = False):
    ...
    current = expressions
    result = None
    while not scheme_nullp(current):
...
def do_cond_form(expressions, env, tail = False):
    ...
    while expressions is not nil:
        ...
        print("DEBUG: clause = {}".format(clause))
        ...
        if clause.first == 'else':
            ...
            test = scheme_eval(clause.first, env, tail)

        # print("DEBUG: test = {}".format(test))

        if is_scheme_true(test):
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            if scheme_nullp(clause.rest):
                return test
            return eval_all(clause.rest, env, tail=True)
...
def do_let_form(expressions, env, tail = False):
    ...
    return eval_all(expressions.rest, let_env, tail=True)
...
def make_let_frame(bindings, env):
    ...
    formals = Pair.map(bindings, lambda x: x.first)
    validate_formals(formals)
    ...
    # while not scheme_nullp(current):
        ...
        # new_frame.define(symbol, scheme_eval(expression, env))
        # current = current.rest
