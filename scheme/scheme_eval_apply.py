...
def scheme_eval(expr, env, tail = False): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        args = rest.map(lambda x: scheme_eval(x, env, tail))

        if tail:
            return Unevaluated(Pair(operator, args), env)
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        # BEGIN PROBLEM 2
        "*** YOUR CODE HERE ***"
        args_to_py_list = lambda x: [x.first] + args_to_py_list(x.rest) if isinstance(x, Pair) else []
        py_args = args_to_py_list(args)
        # END PROBLEM 2
        try:
            # BEGIN PROBLEM 2
            "*** YOUR CODE HERE ***"
...
def eval_all(expressions, env, tail=False):
    ...
    result = None
    current = expressions
    while not scheme_nullp(current.rest):
        result = scheme_eval(current.first, env)
        current = current.rest

    if tail:
        return Unevaluated(current.first, env)
    return scheme_eval(current.first, env, True) # replace this with lines of your own code
    # return result
    # END PROBLEM 6
