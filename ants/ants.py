...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while  current_place and not current_place.is_hive:
            if distance <= self.upper_bound and distance >= self.lower_bound and current_place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.place:
            damage = amount + (self.damage if will_die else 0)
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        ...
        damage = self.base_damage * (0.75 ** distance) - (self.insects_shot * 0.0625)
    ...
    def action(self, gamestate):
        ...
        # print(f"DEBUG: {insects_and_distances}")
        for insect, distance in insects_and_distances.items():
            # print(f"DEBUG: insect = {insect}")
            damage = self.calculate_damage(distance)
            print(f"DEBUG: damage = {damage}")
            if damage:
                insect.reduce_health(damage)
                self.insects_shot += 1
                # self.base_damage -= 1 / 16
                # self.base_damage = max(0, self.base_damage)
